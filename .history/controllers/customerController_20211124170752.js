    ...
    try {
        ...
        if (customer.Customer_Properties != false) {
            for (let i = 0; i < customer.Customer_Properties.length; i++) {
                if (customer.Customer_Properties[i].offer != null) {
                    var offers = [];
    ...
    try {
        ...
        await sequelize.transaction(async (t) => {
           ...
           // console.log(req.body.payment.payment_via.name == "Cheque");
            ...
            }else if(req.body.property.payment_type =='Full'){

                let fullDetails = {
                    receiving_amount : req.body.booking_amount,
                    receiving_amount_date:Date.now(),
                    customer_property_id: customerProperty.id,
                    user_id: req.profile.id,
                    received_amount:req.body.booking_amount,
                }  
                let payments = await Payment.craete(fullDetails, { transaction: t});  
